...
class Model_Notification extends Model
{
    ...
    function getNotification(){
      $request =DB::select('SELECT * FROM sales.notifications WHERE store_id = (Select store_id from sales.staffs where staff_id = ?) and status = 0 ORDER BY date DESC',[session()->get('id')]);
      $array=array();
